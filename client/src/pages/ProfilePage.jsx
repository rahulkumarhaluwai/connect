  ...
  return (
    ...
    flex items-center justify-between max-sm:flex-col-reverse rounded-lg'>
      ...
      <label htmlFor="avatar" className='flex items-center gap-3 cursor-pointer'>
        ...
        <img src={selectedImg ? URL.createObjectURL(selectedImg): authUser?.profilePic} className={`w-12 h-12 rounded-full ${selectedImg && 'rounded-full'}`}/>
        upload profile page
      </label>
      ...
      <img className={`max-w-44 aspect-square rounded-full mx-10 max-sm:mt-10 ${selectedImg && 'rounded-full'}`} src={authUser?.profilePic || assets.logo}/>
      </div>
    </div>
  )
}
